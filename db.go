...
func (s *server) GetChatwootConfig(userID string) (*ChatwootConfig, error) {
	...
	err := s.db.Get(&config, "SELECT * FROM chatwoot_config WHERE user_id=$1", userID)
	if err != nil {
		return nil, err
	}
	return &config, nil
...
func (s *server) SaveChatwootConfig(config *ChatwootConfig) error {
	var query string
	if s.db.DriverName() == "postgres" {
		query = `INSERT INTO chatwoot_config (user_id, account_id, token, url, inbox_id, import_messages)
                 VALUES (:user_id, :account_id, :token, :url, :inbox_id, :import_messages)
                 ON CONFLICT(user_id) DO UPDATE SET
                 account_id=:account_id, token=:token, url=:url, inbox_id=:inbox_id, import_messages=:import_messages`
	} else {
		query = `INSERT INTO chatwoot_config (user_id, account_id, token, url, inbox_id, import_messages)
                 VALUES (:user_id, :account_id, :token, :url, :inbox_id, :import_messages)
                 ON CONFLICT(user_id) DO UPDATE SET
                 account_id=excluded.account_id, token=excluded.token, url=excluded.url, inbox_id=excluded.inbox_id, import_messages=excluded.import_messages`
	}
